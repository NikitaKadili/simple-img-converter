...
void SaveImageLineToJSAMPROW(JSAMPROW row_pointer, int y, const Image& image) {
    const Color* line = image.GetLine(y);
    for (int x = 0; x < image.GetWidth(); ++x) {
        JSAMPLE* pixel = row_pointer + x * 3;
...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    // Right method for C-style file openening for MVS (_MSC_VER) and others
    // Return false if file wasn't opened
#ifdef _MSC_VER
    ...
    for (int y = 0; y < image.GetHeight(); ++y) {
        SaveImageLineToJSAMPROW(buffer[0], y, image);
